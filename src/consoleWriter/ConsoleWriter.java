...
public class ConsoleWriter {
	...
	public void InvoiceConsolePrinter(List<Invoice> invoices) {
		...
		for(Invoice a : invoices) {
			...
			discounttemp = 0;
			subtemp = a.getProductsSubtotal();
			subtotal += subtemp;
			feetemp = a.getFee();
			fee += feetemp;
			taxtemp = a.getTax();
			tax += taxtemp;
			if(a.getCustomer().isStudent()) {
				discounttemp = a.getProuductsDiscount(subtemp);
			...
			System.out.println(String.format("%-10s %-25s %-7s %-20s $%8.2f $%8.2f $%8.2f $(%7.2f) $%8.2f", a.getInvoiceCode(), a.getCustomer().getName(), a.getCustomer().getType(), a.getSalesperson().getName(), subtemp, feetemp, taxtemp, discounttemp, totaltemp));	
		...
		for(Invoice a : invoices) {
			...
			System.out.println("Invoice " + a.getInvoiceCode());
			System.out.println("=============================");
			
			System.out.println("Salesperson: "+ a.getSalesperson().getName());
			...
			for(Product b : a.getProductList()) {
				...
				if(b.getType().equals("SeasonPass")) {
					cost = b.computeCost(a.getInvoiceDate());
